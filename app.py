...
def _exclude_caf_vip(df: pd.DataFrame) -> pd.DataFrame:
    """Usuń wiersze z PosName zawierającym 'CAF' lub 'VIP' (dowolne kino)."""
...
def _keep_caf(df: pd.DataFrame) -> pd.DataFrame:
    """Zostaw tylko wiersze z PosName zawierającym 'CAF' (dowolne kino)."""
    if "PosName" in df.columns:
...
def _keep_vip(df: pd.DataFrame) -> pd.DataFrame:
    """Zostaw tylko wiersze z PosName zawierającym 'VIP' (dowolne kino)."""
    if "PosName" in df.columns:
...
with tab_pivot:
    ...
    if "PosName" in tx_df.columns:
        m_ex = tx_df["PosName"].astype(str).str.contains("CAF|VIP", case=False, regex=True, na=False)
...
with tab_best:
    ...
    tx_df = dff.copy()
    if "PosName" in tx_df.columns:
        mask_excl = tx_df["PosName"].astype(str).str.contains("CAF|VIP", case=False, regex=True, na=False)
