...
with tab_indy:
    ...
    try:
        ...
        # % Zestawy (osoba; bar-only dff_u)
        try:
            sets_u_sum = float(dff_u.loc[dff_u["ProductName"].astype(str).isin(SET_PRODUCTS), "Quantity"].sum())
            tx_count_user_bar = int(dff_u["TransactionId"].nunique()) if "TransactionId" in dff_u.columns else 0
            pct_sets_u = (sets_u_sum / tx_count_user_bar * 100) if tx_count_user_bar else None
        except Exception:
            pct_sets_u = None
