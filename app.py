...
with tab_dane:
    ...
    if st.button("🔄 Wczytaj/odśwież"):
        if not uploaded:
            ...
            for uf in uploaded:
                try:
                    df_tmp = read_any_table(uf)
                    df_tmp["__source_file"] = uf.name
            ...
            if fails:
                st.error("Błędy przy wczytywaniu:")
                for err in fails:
                    st.write("- ", err)

# ---------- Wspólne: filtr dat ----------
def date_filtered(df: pd.DataFrame, key: str) -> pd.DataFrame:
...
with tab_pivot:
    ...
    # Transakcje i średnia transakcji
    ...
    # Średnia kina (globalnie, bez POS CAF/VIP)
...
with tab_indy:
    ...
    if not users_all:
        ...
        def _fmt(v, money=False):
            if pd.isna(v): return ""
            return fmt_pln(v) if money else (f"{v:.1f} %" if isinstance(v,(int,float)) else v)
        formatted = disp.copy()
        formatted[user] = [ _fmt(v, money=(i==0)) for i,v in enumerate(disp[user]) ]
        formatted["Średnia kina"] = [ _fmt(v, money=(i==0)) for i,v in enumerate(disp["Średnia kina"]) ]
...
with tab_best:
    ...
    def rank_series(name, series, avg=None, percent=False):
        dfv = pd.DataFrame({"Zleceniobiorca": series.index, name: series.values})
        dfv = dfv.sort_values(name, ascending=False, na_position="last").reset_index(drop=True)
        if percent:
            dfv[name] = dfv[name].map(lambda v: "" if pd.isna(v) else f"{float(v):.1f} %")
        else:
            dfv[name] = dfv[name].map(lambda v: "" if pd.isna(v) else fmt_pln(float(v)))
        if avg is not None:
            st.caption(f"Średnia kina: {'' if avg is None else (f'{avg:.1f} %' if percent else fmt_pln(avg))}")
    ...
    avg_extra = None if base.sum()==0 else float(extra.sum()/base.sum()*100)
    st.markdown("#### % Extra Sos")
    rank_series("% Extra Sos", pct_extra, avg=avg_extra, percent=True)
    ...
    avg_pop = None if basep.sum()==0 else float(flav.sum()/basep.sum()*100)
    st.markdown("#### % Popcorny smakowe")
    rank_series("% Popcorny smakowe", pct_pop, avg=avg_pop, percent=True)
    ...
    avg_share = None if den.sum()==0 else float(num.sum()/den.sum()*100)
    st.markdown("#### % ShareCorn")
    rank_series("% ShareCorn", pct_share, avg=avg_share, percent=True)
    ...
    if {"TransactionId","NetAmount"}.issubset(dff.columns):
        ...
        rank_series("Średnia wartość transakcji", avg_by_user, avg=None, percent=False)
    else:
...
with tab_comp:
    ...
    # konfigurator (dynamiczne pozycje)
    ...
    if st.button("🧮 Oblicz ranking", type="primary"):
        ...
        # Format wyników
        disp["Wynik (%)"] = disp["Wynik (%)"].map(lambda v: "" if pd.isna(v) else f"{float(v):.1f} %")
...
with tab_cafe:
    ...
    if tx_df.empty:
        ...
        per_tx_total = grp.first().where(grp.nunique(dropna=True) <= 1, grp.sum(min_count=1))
